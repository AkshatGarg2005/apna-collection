// src/pages/WeddingProducts.js
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import styled from 'styled-components';
import { FaEdit, FaTrash, FaPlus, FaSearch, FaRing, FaCircle } from 'react-icons/fa';
...
const WeddingProducts = () => {
  ...
  // Toggle wedding status
  const toggleWeddingStatus = async (productId) => {
    try {
      const productToUpdate = products.find(product => product.id === productId);
      const isWedding = !productToUpdate.isWedding;
  ...
  return (
    <DashboardLayout title="Wedding Collection Management">
      <WeddingHeader>
        <h1>Manage Wedding Collection</h1>
        <p>Mark products to be shown in the Wedding Collection on the website.</p>
      </WeddingHeader>
          ...
          <ProductsGrid>
            {filteredProducts.map(product => (
              <ProductCard key={product.id}>
                ...
                <WeddingTag 
                  onClick={() => toggleWeddingStatus(product.id)}
                  className={product.isWedding ? 'active' : ''}
                >
                  {product.isWedding ? <FaRing /> : <FaCircle className="inactive-ring" />}
                  <span>{product.isWedding ? 'In Wedding Collection' : 'Add to Wedding'}</span>
                </WeddingTag>
                <ProductActions>
                  <ActionButton 
                    className="edit"
...
const WeddingTag = styled.div`
  ...
  svg {
    font-size: 16px;
    color: ${props => props.className === 'active' ? 'white' : '#8e44ad'};
  }
